...
export function attachRolePromptsCommand(root: any): void {
  const cmd = root.command("role-prompts").description("Manage role prompts (list/create/update/delete)");

  cmd
    .command("list")
    .description("List all role prompts")
    .action(async () => {
      const flags = root.opts() as any;
      try {
        const d = await discover(flags);
        const client = createClient(d, flags);
        const res = await client.get("/api/v1/prompts/role");
        ...
        if (rows.length === 0) {
          console.log("No role prompts found");
          process.exit(0);
        }
  ...
  cmd
    ...
    .action(async (opts: { name: string; content: string; id?: string }) => {
      ...
      try {
        ...
        const d = await discover(flags);
        const client = createClient(d, flags);
        const res = await client.post("/api/v1/prompts/role", {
          id: opts.id,
        ...
        console.log(`Created role prompt '${data.name}' (${data.id})`);
        process.exit(0);
      } catch (error) {
        const mapped = handleAxiosError(error, flags);
        if (flags.json && mapped.json) printJsonOrText(mapped.json, flags);
  ...
  cmd
    ...
    .description("Update a role prompt")
    .action(async (id: string, opts: { name: string; content: string }) => {
      ...
      try {
        ...
        const d = await discover(flags);
        const client = createClient(d, flags);
        const res = await client.put(`/api/v1/prompts/role/${encodeURIComponent(id)}`, {
          name: String(opts.name),
          content: String(content)
        });
  ...
  cmd
    ...
    .action(async (id: string) => {
      ...
      try {
        const d = await discover(flags);
        const client = createClient(d, flags);
        const res = await client.delete(`/api/v1/prompts/role/${encodeURIComponent(id)}`);
        const data = (res.data?.data ?? res.data) as boolean;
        if (flags.json) {
          printJsonOrText({ ok: data }, flags);
          process.exit(0);
